...
const updateOrderStatus = async (req, res) => {
  try {
    ...
    const isValidData =
      _.isBoolean(pushNotification) &&
      ['PENDING', 'PROCESSING', 'TRANSFERRING', 'DONE', 'REJECTED'].includes(
        status
      );
    if (!isValidData) {
    ...
    if (pushNotification) {
      axios
        .get(
          `${config.PUBLIC_WEB_CUSTOMER_URL}/mail-service/orders/${order._id}?status=${status}&secretKey=${config.SECRET_KEY}`
        )
    ...
    if (status === 'DONE') {
      ...
      products.forEach((product) => {
        if (mongoose.isValidObjectId(product.productId)) {
          ProductModel.findByIdAndUpdate(product.productId, {
