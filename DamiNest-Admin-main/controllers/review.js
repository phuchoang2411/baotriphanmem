...
const publishReview = async (req, res) => {
  try {
    ...
    const reviewUpdated = await ProductReviewModel.findByIdAndUpdate(
      reviewId,
      { $set: { published: true } },
      { new: true }
    ).exec();
    ...
    axios
      .get(
        `${config.PUBLIC_WEB_CUSTOMER_URL}/mail-service/product-review/${reviewId}?to=owner&secretKey=${config.SECRET_KEY}`
      )
